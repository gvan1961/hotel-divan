...
@Component({
  ...
  <p><strong>TOTAL DE HÓSPEDES:</strong> {{ calcularTotalHospedes() }}</p>
  ...
  styles: [`
    * {
    ...
    /* ⭐ TABELA - OTIMIZADA PARA 50 APARTAMENTOS */
    table {
      width: 100%;
    ...
    @media print {
      ...
      /* ⭐ TABELA SUPER COMPACTA PARA IMPRESSÃO */
      table {
        font-size: 8px; /* ⭐ MUITO PEQUENO */
        page-break-inside: auto;
      }

      th {
        padding: 3px 2px; /* ⭐ MÍNIMO */
        font-size: 8px;
        line-height: 1.1;
      }

      td {
        padding: 2px 3px; /* ⭐ MÍNIMO */
        font-size: 8px;
        line-height: 1.2;
      }
...
calcularTotalHospedes(): number {
  return this.apartamentos.reduce((total, apt) => total + (apt.quantidadeHospedes || 0), 0);
}
