...
public interface ControleDiariaRepository extends JpaRepository<ControleDiaria, Long> {
    ...
    @Query("SELECT cd FROM ControleDiaria cd WHERE cd.dataLancamento < :dataLimite AND cd.dataLancamento >= :dataMinima AND cd.status = 'LANCADA' AND cd.reserva.status = 'ATIVA'")
    List<ControleDiaria> findDiariasParaFechar(
